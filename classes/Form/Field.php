...
class Form_Field
{
    ...
    {
        $html = OwnString::BLANK;

        if( !is_array($options) )
        {
            $options = explode(',', OwnString::FormatCommaSeparated($options));
        }
        ...
        foreach( $options as $option )
        {
            $html .= '<option value="' . htmlspecialchars($option, ENT_QUOTES) . '"' . ($option == $selected ? ' selected="selected"' : '') . '>' .
                     htmlspecialchars($option) . '</option>' . OwnString::NEWLINE_UNIX;
        }
    ...
    public static function Options($options, $selected = null, $index_value = null, $index_text = null, $max_length = null)
    {
        ...
        foreach( $options as $key => $val )
        {
            ...
            if( $value === self::OPTGROUP )
            {
                if( $in_optgroup )
                {
                    $html .= '</optgroup>' . OwnString::NEWLINE_UNIX;
                }

                $html .= '<optgroup label="'.$val[$index_text].'">' . OwnString::NEWLINE_UNIX;
            }
            else
            {
                $html .= '<option ' .
                         ...
                         htmlspecialchars(OwnString::Truncate($simple_options ? $val : $val[$index_text], $max_length)) .
                         '</option>' . OwnString::NEWLINE_UNIX;
            }
        }
