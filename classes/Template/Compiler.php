...
class Template_Compiler
{
    ...
    public static function Compile($template)
    {
        ...
        $template = OwnString::FormatNewlines($template);

        // Mark with line numbers
        $line_number = 0;
        $marked_template = '';
        foreach( explode(OwnString::NEWLINE_UNIX, $template) as $line )
        {
            $line_number++;
            $marked_template .= preg_replace('~{([/$a-z"]+)~i', "(*$line_number*){\\1", $line) . OwnString::NEWLINE_UNIX;
        }
        unset($template);
        ...
        try
        {
            ...
            switch( $tag['tag'] )
            {
                ...
                case '/foreach':
                    ...
                    return self::PHP_START . OwnString::NEWLINE_UNIX .
                           '    endforeach;' . OwnString::NEWLINE_UNIX .
                           'endif;' . OwnString::NEWLINE_UNIX .
                           self::PHP_END;
    ...
    private static function CompileReasonsTag($attributes)
    {
        ...
        return self::PHP_START .
               "\$DB = GetDB();" . OwnString::NEWLINE_UNIX .
               $attributes['var'] . " = \$DB->FetchAll('SELECT * FROM `tbx_reason` WHERE `type`=? ORDER BY `short_name`', array('" . $attributes['type'] . "'));" . OwnString::NEWLINE_UNIX .
               self::PHP_END;
    }
    ...
    private static function CompileOptionsTag($attributes)
    {
        ...
        return self::PHP_START .
               "foreach( " . $attributes['from'] . " as \$x_options )" . OwnString::NEWLINE_UNIX .
               "{" . OwnString::NEWLINE_UNIX .
               'echo "<option value=\"" . htmlspecialchars($x_options[\'' . $attributes['key'] . '\']) . "\"" . ' . OwnString::NEWLINE_UNIX .
               '($x_options[\'' . $attributes['key'] . '\'] == ' . $attributes['selected'] . ' ? " selected=\"selected\"" : "") . ' . OwnString::NEWLINE_UNIX .
               '">" . htmlspecialchars($x_options[\'' . $attributes['value'] . '\']) . "</option>";' . OwnString::NEWLINE_UNIX .
               "}" . OwnString::NEWLINE_UNIX .
               self::PHP_END;
    }
    ...
    private static function CompileUserTag($attributes)
    {
        ...
        return self::PHP_START . OwnString::NEWLINE_UNIX .
               '$DB = GetDB();' . OwnString::NEWLINE_UNIX .
               $attributes['var'] . " = \$DB->Row('$query', array(" . $attributes['username'] . "));" . OwnString::NEWLINE_UNIX .
               self::PHP_END;
    }
    ...
    private static function CompilePlayerTag($attributes)
    {
        ...
        return self::PHP_START . OwnString::NEWLINE_UNIX .
               "\$x_clip =& " . $attributes['clips'] . "[0];" . OwnString::NEWLINE_UNIX .
               "\$this->vars['g_clip'] =& \$x_clip;" . OwnString::NEWLINE_UNIX .
               "if( \$x_clip['type'] == 'Embed' )" . OwnString::NEWLINE_UNIX .
               "{" . OwnString::NEWLINE_UNIX .
               "echo \$x_clip['clip'];" . OwnString::NEWLINE_UNIX .
               "}" . OwnString::NEWLINE_UNIX .
               "else if( preg_match('~\\.' . FLASH_EXTENSIONS . '\$~i', \$x_clip['clip']) )" . OwnString::NEWLINE_UNIX .
               "{" . OwnString::NEWLINE_UNIX .
               "include(TEMPLATE_COMPILE_DIR . '/" . $attributes['flv'] . "');" . OwnString::NEWLINE_UNIX .
               "}" . OwnString::NEWLINE_UNIX .
               "else if( preg_match('~\\.' . QT_EXTENSIONS . '\$~i', \$x_clip['clip']) )" . OwnString::NEWLINE_UNIX .
               "{" . OwnString::NEWLINE_UNIX .
               "include(TEMPLATE_COMPILE_DIR . '/" . $attributes['qt'] . "');" . OwnString::NEWLINE_UNIX .
               "}" . OwnString::NEWLINE_UNIX .
               "else if( preg_match('~\\.' . WM_EXTENSIONS . '\$~i', \$x_clip['clip']) )" . OwnString::NEWLINE_UNIX .
               "{" . OwnString::NEWLINE_UNIX .
               "include(TEMPLATE_COMPILE_DIR . '/" . $attributes['wmv'] . "');" . OwnString::NEWLINE_UNIX .
               "}" . OwnString::NEWLINE_UNIX .
               "else" . OwnString::NEWLINE_UNIX .
               "{" . OwnString::NEWLINE_UNIX .
               "include(TEMPLATE_COMPILE_DIR . '/" . $attributes['other'] . "');" . OwnString::NEWLINE_UNIX .
               "}" . OwnString::NEWLINE_UNIX .
               self::PHP_END;
    }
    ...
    private static function CompileCommentsTag($attributes)
    {
        ...
        if( $attributes['paginate'] === true )
        {
            ...
            return self::PHP_START . OwnString::NEWLINE_UNIX .
                   '$DB = GetDB();' . OwnString::NEWLINE_UNIX .
                   "\$x_result = \$DB->QueryWithPagination('$query', array(" . join(',', $binds) . "), " . $attributes['page'] . ", " . $attributes['amount'] . ");" . OwnString::NEWLINE_UNIX .
                   (isset($attributes['pagination']) ? $attributes['pagination'] . ' = $x_result;' . OwnString::NEWLINE_UNIX : '') .
                   $attributes['var'] . " = \$DB->FetchAll(\$x_result['handle']);" . OwnString::NEWLINE_UNIX .
                   self::PHP_END;
        }
        else
        {
            ...
            return self::PHP_START . OwnString::NEWLINE_UNIX .
                   '$DB = GetDB();' . OwnString::NEWLINE_UNIX .
                   $attributes['var'] . " = \$DB->FetchAll('$query', array(" . join(',', $binds) . "));" . OwnString::NEWLINE_UNIX .
                   self::PHP_END;
        }
    }
    ...
    private static function CompileClipsTag($attributes)
    {
        ...
        return self::PHP_START . OwnString::NEWLINE_UNIX .
               '$DB = GetDB();' . OwnString::NEWLINE_UNIX .
               $attributes['var'] . " = \$DB->FetchAll('$query', array(" . $attributes['videoid'] . "));" . OwnString::NEWLINE_UNIX .
               "\$x_thumbs = \$DB->FetchAll('SELECT * FROM `tbx_video_thumbnail` WHERE `video_id`=? ORDER BY `thumbnail_id`', array(" . $attributes['videoid'] . "));" . OwnString::NEWLINE_UNIX .
               "\$x_offset = round(count(\$x_thumbs)/count(" . $attributes['var'] . "));" . OwnString::NEWLINE_UNIX .
               "foreach( " . $attributes['var'] . " as \$x_i => \$x_clip )" . OwnString::NEWLINE_UNIX .
               "{" . OwnString::NEWLINE_UNIX .
               $attributes['var'] . "[\$x_i]['thumbnail'] = \$x_thumbs[\$x_i * \$x_offset]['thumbnail'];" . OwnString::NEWLINE_UNIX .
               "}" . OwnString::NEWLINE_UNIX .
               self::PHP_END;
    }
    ...
    private static function CompileStatsTag($attributes)
    {
        ...
        return self::PHP_START . OwnString::NEWLINE_UNIX .
               '$DB = GetDB();' . OwnString::NEWLINE_UNIX .
               $attributes['var'] . "['users'] = \$DB->QueryCount('SELECT COUNT(*) FROM `tbx_user` WHERE `status`=?', array('Active'));"  . OwnString::NEWLINE_UNIX .
               $attributes['var'] . "['videos'] = \$DB->QueryCount('SELECT COUNT(*) FROM `tbx_video` WHERE `status`=? AND `is_private`=0', array('Active'));"  . OwnString::NEWLINE_UNIX .
               $attributes['var'] . "['categories'] = \$DB->QueryCount('SELECT COUNT(*) FROM `tbx_category`');"  . OwnString::NEWLINE_UNIX .
               $attributes['var'] . "['comments'] = \$DB->QueryCount('SELECT COUNT(*) FROM `tbx_video_comment` WHERE `status`=?', array('Active'));"  . OwnString::NEWLINE_UNIX .
               self::PHP_END;
    }
    ...
    private static function CompileBannerTag($attributes)
    {
        ...
        return self::PHP_START . OwnString::NEWLINE_UNIX .
               '$DB = GetDB();' . OwnString::NEWLINE_UNIX .
               "\$x_banner = \$DB->Row('$query', array(" . join(',', $binds) . "));" . OwnString::NEWLINE_UNIX .
               "\$DB->Update('UPDATE `tbx_banner` SET `times_displayed`=`times_displayed`+1 WHERE `banner_id`=?', array(\$x_banner['banner_id']));" . OwnString::NEWLINE_UNIX .
               "echo \$x_banner['banner_html'];" . OwnString::NEWLINE_UNIX .
               self::PHP_END;
    }
    ...
    private static function CompileSponsorTag($attributes)
    {
        ...
        return self::PHP_START . OwnString::NEWLINE_UNIX .
               '$DB = GetDB();' . OwnString::NEWLINE_UNIX .
               $attributes['var'] . " = \$DB->Row('$query', array(" . $attributes['sponsorid'] . "));" . OwnString::NEWLINE_UNIX .
               self::PHP_END;
    }
    ...
    private static function CompileSponsorsTag($attributes)
    {
        ...
        return self::PHP_START . OwnString::NEWLINE_UNIX .
               '$DB = GetDB();' . OwnString::NEWLINE_UNIX .
               $attributes['var'] . " = \$DB->FetchAll('$query', array(" . join(',', $binds) . "));"  . OwnString::NEWLINE_UNIX .
               ($attributes['alphabetize'] ? "usort(" . $attributes['var'] . ", create_function('\$a, \$b', 'return strcmp(\$a[\\'name\\'], \$b[\\'name\\']);'));". OwnString::NEWLINE_UNIX : '') .
               self::PHP_END;
    }
    ...
    private static function CompileVideoTag($attributes)
    {
        ...
        return self::PHP_START . OwnString::NEWLINE_UNIX .
               '$DB = GetDB();' . OwnString::NEWLINE_UNIX .
               "if( !isset(\$this->vars['g_private']) )" . OwnString::NEWLINE_UNIX .
               '{' . OwnString::NEWLINE_UNIX .
               $attributes['var'] . " = \$DB->Row('$query', array('Active', " . $attributes['videoid'] . "));" . OwnString::NEWLINE_UNIX .
               '}' . OwnString::NEWLINE_UNIX .
               "else if( !\$this->vars['g_private'] )" . OwnString::NEWLINE_UNIX .
               '{' . OwnString::NEWLINE_UNIX .
               $attributes['var'] . " = \$DB->Row('$npquery', array(0, 'Active', " . $attributes['videoid'] . "));" . OwnString::NEWLINE_UNIX .
               '}' . OwnString::NEWLINE_UNIX .
               'else' . OwnString::NEWLINE_UNIX .
               '{' . OwnString::NEWLINE_UNIX .
               $attributes['var'] . " = \$DB->Row('$pquery', array(1, 'Active', \$this->vars['g_private_id'], " . $attributes['videoid'] . "));" . OwnString::NEWLINE_UNIX .
               '}' . OwnString::NEWLINE_UNIX .
               self::PHP_END;
    }
    ...
    private static function CompileCategoryTag($attributes)
    {
        ...
        return self::PHP_START . OwnString::NEWLINE_UNIX .
               '$DB = GetDB();' . OwnString::NEWLINE_UNIX .
               $attributes['var'] . " = \$DB->Row('$query', array(" . join(',', $binds) . "));" . OwnString::NEWLINE_UNIX .
               self::PHP_END;
    }
    ...
    private static function CompileSearchTermsTag($attributes)
    {
        ...
        {
            $queries = "\$x_count = max(1, \$DB->QueryCount('SELECT COUNT(*) FROM `tbx_search_term`'));" . OwnString::NEWLINE_UNIX .
                       "\$DB->QuerySingleColumn('SELECT @MAX:=`frequency` FROM `tbx_search_term` ORDER BY `frequency` DESC LIMIT 1');" . OwnString::NEWLINE_UNIX .
                       "\$DB->QuerySingleColumn('SELECT @MIN:=`frequency` FROM `tbx_search_term` ORDER BY `frequency` DESC LIMIT ' . (min(\$x_count, " . $attributes['amount'] . ") - 1) . ',1');" . OwnString::NEWLINE_UNIX .
        ...
        return self::PHP_START . OwnString::NEWLINE_UNIX .
               '$DB = GetDB();' . OwnString::NEWLINE_UNIX .
               'Blacklist::FilterSearchTerms();' . OwnString::NEWLINE_UNIX .
               $queries . OwnString::NEWLINE_UNIX .
               ($attributes['alphabetize'] ? "usort(" . $attributes['var'] . ", create_function('\$a, \$b', 'return strcmp(\$a[\\'term\\'], \$b[\\'term\\']);'));". OwnString::NEWLINE_UNIX : '') .
               self::PHP_END;
    }
    ...
    private static function CompileTagsTag($attributes)
    {
        ...
        if( !empty($attributes['amount']) )
        {
            $queries = "\$x_count = max(1, \$DB->QueryCount('SELECT COUNT(*) FROM `tbx_video_tag`'));" . OwnString::NEWLINE_UNIX .
                       "\$DB->QuerySingleColumn('SELECT @MAX:=`frequency` FROM `tbx_video_tag` ORDER BY `frequency` DESC LIMIT 1');" . OwnString::NEWLINE_UNIX .
                       "\$DB->QuerySingleColumn('SELECT @MIN:=`frequency` FROM `tbx_video_tag` ORDER BY `frequency` DESC LIMIT ' . (min(\$x_count, " . $attributes['amount'] . ") - 1) . ',1');" . OwnString::NEWLINE_UNIX .
                       $attributes['var'] . " = \$DB->FetchAll('SELECT *,IF(@MAX=@MIN, 100, " .
                          "ROUND(((`frequency`-@MIN) * (" . $attributes['maxscore'] . "-" . $attributes['minscore'] . ")/(@MAX-@MIN) + " . $attributes['minscore'] . ")/10) * 10) AS `score` " .
        ...
        return self::PHP_START . OwnString::NEWLINE_UNIX .
               '$DB = GetDB();' . OwnString::NEWLINE_UNIX .
               $queries . OwnString::NEWLINE_UNIX .
               ($attributes['alphabetize'] ? "usort(" . $attributes['var'] . ", create_function('\$a, \$b', 'return strcmp(\$a[\\'tag\\'], \$b[\\'tag\\']);'));". OwnString::NEWLINE_UNIX : '') .
               self::PHP_END;
    }
    ...
    private static function CompileTemplateTag($attributes)
    {
        ...
        return self::PHP_START . OwnString::NEWLINE_UNIX .
               ...
               $variables .
               "include(TEMPLATE_COMPILE_DIR . '/" . $attributes['file'] . "');" . OwnString::NEWLINE_UNIX .
               self::PHP_END;
    }
    ...
    private static function CompileForeachStart($attributes)
    {
        ...
        return self::PHP_START . OwnString::NEWLINE_UNIX .
               'if( is_array(' . $attributes['from'] . ') ):' . OwnString::NEWLINE_UNIX .
               ($attributes['counter'] ? '    ' . $attributes['counter'] . ' = 0;' . OwnString::NEWLINE_UNIX : '') .
               '    foreach( ' . $attributes['from'] . ' as ' . $attributes['var'] . ' ):' . OwnString::NEWLINE_UNIX .
               ($attributes['counter'] ? '    ' . $attributes['counter'] . '++;' . OwnString::NEWLINE_UNIX : '') .
               self::PHP_END;
    }
    ...
    private static function VerifyRequiredAttributes($required, &$attributes, $tag)
    {
        ...
        {
            if( !isset($attributes[$r]) || OwnString::IsEmpty($attributes[$r]) )
            {
