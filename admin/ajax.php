...
function tbxVideoAdd()
{
    ...
    if( !OwnString::IsEmpty($_REQUEST['username']) )
    {
    ...
    $_REQUEST['date_recorded'] = OwnString::Nullify($_REQUEST['date_recorded']);
    $_REQUEST['username'] = OwnString::Nullify(Request::Get('username'));
...
function tbxVideoFeedAdd($phase)
{
    switch($phase)
    {
        ...
        case Phase::PRE_INSERT:
            $_REQUEST['username'] = OwnString::Nullify(Request::Get('username'));
            $_REQUEST['sponsor_id'] = OwnString::Nullify(Request::Get('sponsor_id'));
            break;
    }
}
...
function tbxUserAdd($phase)
{
    switch($phase)
    {
        ...
        case Phase::PRE_INSERT:
            ...
            $_REQUEST['date_birth'] = OwnString::Nullify(Request::Get('date_birth'));
...
function tbxUserEdit($phase)
{
    switch($phase)
    {
        ...
        case Phase::PRE_UPDATE:
            ...
            if( !OwnString::IsEmpty($_REQUEST['password']) )
            {
...
function tbxBannerAdd($phase)
{
    switch($phase)
    {
        ...
        case Phase::PRE_INSERT:
            ...
            $_REQUEST['sponsor_id'] = OwnString::Nullify($_REQUEST['sponsor_id']);
...
function tbxEmailTemplateLoad()
{
    ...
    {
        $output['t_subject'] = '';
        $output['t_message'] = OwnString::FormatNewlines(file_get_contents($template));
    }
    else
    {
        ...
        $output['t_message'] = OwnString::FormatNewlines($template_code['message']);
    }
...
function tbxEmailTemplateSearchReplace()
{
    ...
    {
        $search = OwnString::FormatNewlines(Request::Get('search'), OwnString::NEWLINE_UNIX);
        $replace = OwnString::FormatNewlines(Request::Get('replace'), OwnString::NEWLINE_UNIX);
...
function tbxSiteTemplateLoad()
{
    ...
    $output = array();
    $template = TEMPLATES_DIR . '/' . File::Sanitize(Request::Get('template'));
    $output['code'] = OwnString::FormatNewlines(htmlspecialchars(file_get_contents($template)), OwnString::NEWLINE_WINDOWS);
...
function tbxDatabaseQuery()
{
    ...
    if( preg_match('~^(select|show|describe)~i', $query) )
    {
        ...
        for( $i = 0; $i < $fields; $i++ )
        {
            $name = ((($___mysqli_tmp = mysqli_fetch_field_direct($result,  $i)->name) && (!is_null($___mysqli_tmp))) ? $___mysqli_tmp : false);
...
function tbxAutocomplete()
{
    ...
    {
        echo $row[$field] . OwnString::NEWLINE_UNIX;
    }
...
function tbxGenericShowEdit($type)
{
    ...
    $original = $_REQUEST;
    $_REQUEST = OwnString::HtmlSpecialChars($_REQUEST);
...
function tbxGenericSearch()
{
    ...
    // Fulltext searches
    if( isset($_REQUEST['text_search']) && !OwnString::IsEmpty($_REQUEST['text_search']) )
    {
        $columns = array();
    ...
    if( $result['handle'] )
    {
        ...
        while( $original = $DB->NextRow($result['handle']) )
        {
            ...
            $item = OwnString::HtmlSpecialChars($original);
