    ...
    public static function ProcessSource($source)
    {
        ...
        switch($source['source'])
        {
            case self::SOURCE_CLIPBOARD:
                if( OwnString::IsEmpty($_REQUEST[self::FIELD_CLIPBOARD]) )
                {
                    throw new BaseException('The Clipboard field was empty');
                }

                file_put_contents($file, OwnString::FormatNewlines($_REQUEST[self::FIELD_CLIPBOARD]));
                break;
            ...
            case self::SOURCE_URL:
                ...
                {
                    file_put_contents($file,  OwnString::FormatNewlines($http->body));
                }
                else
                {
    ...
    public static function Import($settings)
    {
        ...
        while( !feof($fp) )
        {
            ...
            if( !isset($video['gallery_url']) && !isset($video['thumbnail_url']) && isset($video['base_thumbnail_url']) )
            {
                ...
                foreach( explode(',', OwnString::FormatCommaSeparated($video['thumbnail_filename'])) as $filename )
                {
