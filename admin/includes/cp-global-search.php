        ...
        switch($icon->type->val())
        {
            case 'link':
                echo '<a href="'.$icon->link.'" target="'.$icon->target.'">' .
                     '<img src="images/'.$icon->img.'" border="0" class="toolbar-icon" title="'.$icon->title.'" /></a>' . OwnString::NEWLINE_UNIX;
                break;

            case 'dialog':
                echo '<img src="images/'.$icon->img.'" border="0" class="toolbar-icon" title="'.$icon->title.'" meta="{t: \'dialog\', r: \''.$icon->function.'\'}" />' . OwnString::NEWLINE_UNIX;
                break;

            case 'action':
                echo '<img src="images/'.$icon->img.'" border="0" class="toolbar-icon" title="'.$icon->title.'" meta="{t: \'action\', r: \''.$icon->function.'\'}" />' . OwnString::NEWLINE_UNIX;
                break;

            case 'separator':
            default:
                echo '<span class="toolbar-icon-separator"></span>' . OwnString::NEWLINE_UNIX;
                break;
        }
    }
    ?>
