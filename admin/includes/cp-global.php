...
{
    $xml = OwnString::FormatNewlines($xml, OwnString::NEWLINE_UNIX);
    ...
    $indent_by = 2;
    $output = '';
    foreach( explode(OwnString::NEWLINE_UNIX, $xml) as $line )
    {
        if( preg_match('~^(<[^/][^>]+[^/]>)$~U', $line) )
        {
            $output .= str_repeat(' ', $indent++ * $indent_by) . $line . OwnString::NEWLINE_UNIX;
        }
...
function ResizeableColumn($label, $value, $numeric = false, $joiner = null, $type = null, $link = null)
{
    ...
    return '<span class="resizeable-column">' .
           '<b' . ($overflow ? ' class="clickable overflow"' : '') . '>' . OwnString::Truncate($label, 18) . ':</b> ' .
           $value .
           '</span>';
}
...
function ProgressBarUpdate($id, $percent, $text = 'percent')
{
    ...
    echo "<script language=\"JavaScript\">" .
         "window.parent.\$('#$id').progressbar('option', 'value', $percent);" . OwnString::NEWLINE_UNIX .
         "window.parent.\$('#$id').progressbar('text', '$text');" . OwnString::NEWLINE_UNIX .
         "</script>";
...
function ProgressBarShow($id)
{
    ...
    echo "<script language=\"JavaScript\">" .
         "window.parent.\$('#$id').show();" . OwnString::NEWLINE_UNIX .
         "</script>";
...
function ProgressBarHide($id, $growl = null, $id_button = null, $reset = true)
{
    ...
    echo "<script language=\"JavaScript\">" .
         "window.parent.\$('#$id').hide();" . OwnString::NEWLINE_UNIX .
         "window.parent.\$('img.activity').hide();" . OwnString::NEWLINE_UNIX .
         ($reset ?
         "window.parent.\$('#$id').progressbar('option', 'value', 0);" . OwnString::NEWLINE_UNIX .
         "window.parent.\$('#$id').progressbar('text', '');" . OwnString::NEWLINE_UNIX : '') .
         (!empty($growl) ? "window.parent.\$.growl.message('$growl');" . OwnString::NEWLINE_UNIX : '') .
         (!empty($id_button) ? "window.parent.\$('#$id_button').removeAttr('disabled');" . OwnString::NEWLINE_UNIX : '') .
         "</script>";
...
function IncludeJavascript($file)
{
    if( is_file($file) )
    {
        echo '<script type="text/javascript" language="JavaScript">' . OwnString::NEWLINE_UNIX;
        include($file);
        echo '</script>' . OwnString::NEWLINE_UNIX;
    }
}
